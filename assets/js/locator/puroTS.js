...
puro.locator = {
      ...
      console.log(ele_query)
               ...
               query : function(filter_id){
                                             ...
                                             var url = '/assets/data/' + $(ele_query).val() + '.xml';
                                             ...
                                             $.ajax({
                                                            ...
                                                            // Parse the soap xsl into a json object we can use
                                                            $( response ).find( 'DepotLocation' ).each( function(){
                                                                                          var location = {};
                                                                                          location.LocationName                 = $( this ).find( 'LocationName' ).text();
                                                                                          location.Latitude                                            = $( this ).find( 'Latitude' ).text();
                                                                                          location.Longitude                                         = $( this ).find( 'Longitude' ).text();
                                                                                          location.StreetNumber                  = $( this ).find( 'StreetNumber' ).text();
                                                                                          location.StreetName                      = $( this ).find( 'StreetName' ).text();                                                                   
                                                                                          location.StreetType                        = $( this ).find( 'StreetType' ).text();
                                                                                          location.CityName                                         = $( this ).find( 'CityName' ).text();
                                                                                          location.ProvinceStateCode         = $( this ).find( 'ProvinceStateCode' ).text();
                                                                                          location.PostalCode                       = $( this ).find( 'PostalCode' ).text();
                                                                                          location.Longitude                                         = $( this ).find( 'Longitude' ).text();
                                                                                          location.Longitude                                         = $( this ).find( 'Longitude' ).text();
                                                                                          location.LocationType                   = $( this ).find( 'LocationType' ).text();
                                                                                          location.LocationId                                       = $( this ).find( 'LocationId' ).text();
                                                                                          location.PhoneNumber                  = $( this ).find( 'PhoneNumber' ).text();
                                                            ...
                                                            if (filter_by == false) {
                                                                           ...
                                                                           if (filter_id == "AllLocations")
                                                                           {
                                                                                          ...
                                                                                          console.log("filter");
                                                                                          var newData = { locations : [] };
                                                                                          var n = 0;
                                                                                          for( var i = 0; i < data.locations.length; i++ ){
                                                                                                         //n = i;
                                                                                                         console.log("n: " +n);
                                                                                                         console.log("filter_id: " + filter_id);
                                                                                                         console.log("Location Type: " + data.locations[i].LocationType);
                                                                                                         if ( data.locations[i].LocationType == filter_id) {
                                                                                                                                       console.log("runs" + n);
                                                                                                                                       newData.locations[n] = data.locations[i];
                                                                                                                                       n++;
                                                                                                                        }
                                                                                          }
                                                                                          puro.locator.updateUi( newData );
                                                                                          console.log("newData" + newData.locations.length);
                                                                           }
                                                            }
                                             });
               ...
               updateUi : function( data ){
                              ...
                              try {        
                                             ...
                                             var _header        = data.locations.length + ' Result';
                                             if( data.locations.length > 1 ){
                                                            _header               = _header + 's';
                                             }
                                             _header                                             = _header + ' for "' + $( ele_query ).val() + '"'; 
                                             ...
                                             {
                                                            console.log("No location :" + data.locations.length);
                                             ...
                                             for( var i = 0; i < data.locations.length; i++ ){
                                                            ...
                                                            var location = '<div class="one">' + 
                                                                                          '<div class="medium-offset-1 medium-1 columns infoicons" id="infoicon">' +
                                                                                                         '<img src="' + icon_path +'locator-pin-' + data.locations[ i ].LocationType.toLowerCase() + '.png" id="loinfo" />' +
                                                                                          '</div>' + 
                                                            ...
                                                            markers.push(    new google.maps.Marker({
                                                                                                                                                     position : new google.maps.LatLng( data.locations[ i ].Latitude, data.locations[ i ].Longitude ),
                                                                                                                                                      icon : icon_path + 'locator-pin-' + data.locations[ i ].LocationType.toLowerCase() + '.png'
                                                                                                                                       }) );
                                             ...
                                             if(data.locations.length == 0) 
                                             {
                                                            ...
                                                            var myCenter      = new google.maps.LatLng( data.locations[ 0 ].Latitude, data.locations[ 0 ].Longitude );
                              ...
                              //            info[ i ] = infowindow;
                                             ...
                                             var         info_window_content = '<div style="float: left"><p class="fonts"><strong>' + data.locations[ i ].LocationName +
                                                                                                                                       '</strong></p><p class="fonts">' + data.locations[ i ].StreetNumber + ' ' + data.locations[ i ].StreetName + ', ' + data.locations[ i ].StreetType  + '<br />' 
                                                                                                                                                                                                                       + data.locations[ i ].CityName + ' ' + data.locations[ i ].ProvinceStateCode + ', ' + data.locations[ i ].PostalCode + '</p>' +
                                                                                                                                         '<p class="fonts">Shipping centre<br />' + data.locations[ i ].PhoneNumber + '</p></div><div id="pano" style="width: 250px; height: 200px; float: right;"></div>';
                              ...
                              /*           var myPano = new google.maps.StreetViewPanorama(
                                             ...
                                             //            console.log("markers array here: " + point[0].infowindow);
                                                            ...
                                                            //myCenter = new google.maps.LatLng( data.locations[ i ].Latitude, data.locations[ i ].Longitude );
                                                                           ...
                                                                           console.log("first location" +  data.locations[ 0 ].Latitude + " " + data.locations[ 0 ].Longitude)
                                             ...
                                             $(".one").on( 'click', function() {                                                
                                                            ...
                                                            for(var a = 0; a < data.locations.length; a++) {
                                                                           if ($(this).find("button#title").text() == data.locations[ a ].LocationName) {
                                                                                          showInfo(markers, a);
                                                                           }
                                                            }
                                             });
